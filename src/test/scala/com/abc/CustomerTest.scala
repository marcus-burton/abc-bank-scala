...
class CustomerTest extends FlatSpec with Matchers {
  "Customer" should "statement" in {
    val checkingAccount: Account = new Account(Account.CHECKING, "Henry")
    val savingsAccount: Account = new Account(Account.SAVINGS, "Henry")
  ...
  it should "testOneAccount" in {
    val oscar: Customer = new Customer("Oscar").openAccount(new Account(Account.SAVINGS, "Oscar"))
    oscar.numberOfAccounts should be(1)
  }

  it should "testTwoAccount" in {
    val oscar: Customer = new Customer("Oscar").openAccount(new Account(Account.SAVINGS, "Oscar"))
    oscar.openAccount(new Account(Account.CHECKING,"Oscar"))
